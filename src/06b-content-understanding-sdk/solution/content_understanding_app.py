...
def build_parser() -> argparse.ArgumentParser:
    ...
    invoice = subparsers.add_parser(
        "invoice", help="prebuilt-invoiceで請求書PDFを分析します。"
    )
    invoice.add_argument("--input", type=Path, required=True)
    invoice.add_argument(
        "--output",
        type=Path,
        default=Path("content-understanding-invoice.json"),
    )
    invoice.add_argument(
        "--raw-output",
        type=Path,
    ...
    equipment = subparsers.add_parser(
        "equipment", help="カスタム画像アナライザーで設備ラベルを分析します。"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result, operation_id = analyze_binary_file(
            client,
            analyzer_id,
            args.input,
        )
