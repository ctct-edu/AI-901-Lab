...
1. West US3でプロジェクトがデプロイされている場合はインスタントアクセスモデルが使用できるため、そのまま **プレイグラウンドで開く** ボタンからプレイグラウンドへ進みます。それ以外のリージョンでデプロイされている場合は、 **カスタムデプロイ** ボタンから**1 分あたりのトークン数レート制限** を **[50,000]** に設定してモデルをデプロイします。
...
1. 検索ボックスに `FLUX.20flex` と入力して直接検索します。
...
1. 検索結果から **FLUX.2-flex** を選択し、**デプロイ** ボタンのドロップダウンから **カスタム設定** を選択してデプロイします。1分あたりの要求レート制限はできるだけ小さい値に設定します。

    > 制限値の上限に達していてデプロイできない場合は、別の画像生成モデルを指定してデプロイします。FLUX.2-pro,gpt-image-1-mini,gpt-image-1.5,gpt-image-2などが該当します。
